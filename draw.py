...
def draw_polygons( polygons, screen, color ):
    ...
    while point < len(polygons) - 1:
        if (dot_product(calculate_normal(polygons, point), [0,0,1]) > 0):
...
def add_box( polygons, x, y, z, width, height, depth ):
    ...
    #back
    add_polygon(polygons, x, y, z1, x1, y, z1, x, y, z1)
...
def add_sphere(polygons, cx, cy, cz, r, steps ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop+1):
            ...
            #points for pairs of triangles
            a = index
            b = (index + 1) % len(points)
            c = (index + steps + 1) % len(points)
            d = (index + steps) % len(points)
            #starting triangle
            if longt == 0:
                add_polygon(polygons, points[a][0], points[a][1], points[a][2],
